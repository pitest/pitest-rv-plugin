...
class UOI1MethodVisitor extends MethodVisitor {

    private final MethodMutatorFactory factory;
    private final MutationContext context;

    UOI1MethodVisitor(final MethodMutatorFactory factory, final MutationContext context,
        ...
        if ((opcode == Opcodes.GETFIELD)) {
            if (desc.equals("I")) {
                if (this.shouldMutate("Incremented (a++) integer field " + name)) {
                    ...
                    mv.visitInsn(Opcodes.DUP_X1); // stack = ... [ref.field] [ref] [ref.field]
                    mv.visitInsn(Opcodes.ICONST_1);//stack = ... [ref.field] [ref] [ref.field] [1]
                    mv.visitInsn(Opcodes.IADD); // stack = ... [ref.field] [ref] [ref.field +1]
                    mv.visitFieldInsn(Opcodes.PUTFIELD, owner, name, desc); // stack = ... [ref.field]
                    return;
                }
            }
            if (desc.equals("F")) {
